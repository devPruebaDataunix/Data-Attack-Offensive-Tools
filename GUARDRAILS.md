...
| C11 | **Separación datos/instrucciones** — los **16** agentes que ingieren contenido del target/host comprometido (o mensajes A2A) lo tratan como DATOS, nunca como instrucciones | Soft (prompt), uniforme | bloque "Anti-inyección" en osint-recon, recon-suite, active-recon, web-exploit, web-fuzzing, nuclei, sqlmap, vuln-triage, ai-security, network-exploit, metasploit, post-exploit, lateral-discovery, netexec, sliver, c2-exfil (todos salvo reporting/knowledge-postmortem, que no ingieren contenido del target) | **LLM01 Prompt Injection** |
