...
| C5 | **Validación de esquema del blackboard** — un finding/target sin campos obligatorios dispara feedback correctivo; además invariantes de valor: una hipótesis white-box (`code_ref`) no puede ir `confirmed`/`exploited` sin `evidence`, y (mejora F) un `proof_state` que afirma prueba dinámica (`evidenced`/`proven-by-exploit`) exige `evidence` mientras que `roe-capped` exige una FUENTE | Determinista | `.claude/hooks/validate_blackboard.py` (PostToolUse) + `tools/blackboard.py:validate_engagement` | **LLM05 Improper Output Handling** |
...
| C9 | **Auditoría de coherencia pre-informe** — targets fuera de scope, autorización caducada, y reportabilidad por `proof_state` efectivo (mejora F): un reportable que afirma prueba dinámica sin `evidence`, o cualquier reportable (incl. `roe-capped`) sin FUENTE, es un FALLO; los `roe-capped` se cuentan y avisan (reales, no explotados por ROE — se reportan, no se descartan) | Determinista | `tools/analyze_engagement.py` (reusa `tools/blackboard.py:is_reportable`) | LLM05, LLM09 |
